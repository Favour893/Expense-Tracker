...
export function IncomeExpenseTabs({ value, onChange, panelId, incomeCount, expenseCount, className = "" }: Props) {
  ...
  return (
    <div
      className={`grid grid-cols-2 gap-1.5 sm:gap-2 ${className}`}
      role="tablist"
    ...
    >
      <button
        ...
        className={`min-h-[2.35rem] sm:min-h-[2.85rem] rounded-xl border-2 px-2 py-2 text-center text-[12px] sm:px-3 sm:py-2.5 sm:text-sm font-semibold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-900 ${
          value === "income"
